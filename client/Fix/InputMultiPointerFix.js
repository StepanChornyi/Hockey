import { System, Debug, Vector, Black, InputComponent, KeyInfo, Input} from "black-engine";
/**
 ...
 * @extends System
 */
export class InputMultiPointer extends Input {
  /**
   * Private constructor.
   */
  constructor() {
  ...
  onUpdate() {
    ...
    while (this.mPointerQueue.length > 0) {
      ...
      this.mPointerPositions[nativeEvent.e.pointerId] = this.mPointerPosition.clone();
  ...
  __processNativeEvent(nativeEvent, pos, type) {
    ...
    const info = new PointerInfo(this.mTarget, pos.x, pos.y, nativeEvent.e.button, delta, nativeEvent.e.pointerId);
    this.post(type, info);
  ...
  getPointerPosition(pointerId) {
    return this.mPointerPositions[pointerId];
  }
...
class PointerInfo {
   ...
   */
  constructor(activeObject, x, y, button, delta = 0, id = -1) {
